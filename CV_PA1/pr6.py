...
gh0_org = abs(cv2.Sobel(ih,ddepth = cv2.CV_8U, dx = 1, dy = 0)) + abs(cv2.Sobel(ih,ddepth = cv2.CV_8U, dx = 0, dy = 1))   # np.max(gul_org) = 255, np.min(gul_org) = 0 
...
lap_ih0 = cv2.Laplacian(ih ,ddepth = cv2.CV_8U)   #CV_8U   CV_64F    (256, 256)
...
for i in range(0, iteration):
    lap_iht = cv2.Laplacian(iht, ddepth = cv2.CV_64F)   # (256, 256)
